...
function start_gl(canvas_id, vertexShader, fragmentShader) {
  setTimeout(function () {
    ...
    canvas.onmousedown = function (event) {
        setMouse(event, 1);
      } // On mouse press, set z to 1.
    canvas.onmousemove = function (event) {
      setMouse(event);
    }
    canvas.onmouseup = function (event) {
        setMouse(event, 0);
      } // On mouse press, set z to 0.
...
function gl_init(gl, vertexShader, fragmentShader) {
  ...
  ['uTime', 'uCursor', 'uSpaceDown'].forEach(function (name) {
...
//my little hacky keydown shit
var spaceDown = 0;
window.onkeydown = function (e) {
  if (e.which === 32) {
    e.preventDefault();
    if (spaceDown === 0) spaceDown = 1;
    else if (spaceDown === 1) spaceDown = 0;
...
function gl_update(gl) {
  ...
  gl.uniform1i(gl.uSpaceDown, spaceDown); // Set uniform variables
